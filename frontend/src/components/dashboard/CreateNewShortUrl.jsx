...
const CreateNewShortUrl = ({ setOpen, refetch }) => {
  ...
  const createShortUrlHandler = async (data) => {
    ...
    try {
      ...
      await refetch(); // to update the list of short URLs in the dashboard
      reset();
      setOpen(false);
    } catch (error) {
  ...
  return (
    <div className="flex justify-center items-center bg-white rounded-md">
      ...
      >
        ...
        <button
          className="bg-customRed font-semibold text-white w-32 bg-custom-gradient hover:bg-custom-gradient-hover py-2 transition-colors rounded-md my-3"
          type="submit"
        >
