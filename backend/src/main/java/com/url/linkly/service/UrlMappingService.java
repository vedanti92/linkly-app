...
//            Record click event
    ...
    public void deleteUrl(Long id, User user) {
        UrlMapping urlMapping = urlMappingRepository.findById(id).orElseThrow(() -> new RuntimeException("URL not found."));
    ...
    public UrlMappingDTO updateUrl(Long id, String newOriginalUrl, User user) {
        UrlMapping urlMapping = urlMappingRepository.findById(id).orElseThrow(() -> new RuntimeException("URL not found."));
